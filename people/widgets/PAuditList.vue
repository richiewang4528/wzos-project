    ...
    <div>
        <div class="innernav-wrapper">
            <div style="float:left">
               ...
               <el-button  @click="passTaskAudit()" :loading="passTaskAuditLoading" class="abitem" type="primary" size="mini">任务审核通过</el-button>
               <el-button  @click="notPassTaskAudit()" class="abitem" type="danger" size="mini">任务审核不通过</el-button>
            </div>
        </div>
...
<script>
    import pTable from '../widgets/PAuditTable.vue'
    export default {
        components:{
            pTable
        },
        data(){
            return {
                ...
                setDispatchStr:'set_audit_people_list',
                passTaskAuditLoading: false
            }
        },
        methods: {
            checkCurrentSelection(storageflag){
                let curSelection = this.$store.getters.getTaskSelections
                if(storageflag){
                    curSelection = this.$store.getters.getStorageSelections
                }
                let flag = true
            ...
            invokePassAudit(flagData, passed, dispatchPassStr){
                ...
                this.$store.dispatch(dispatchPassStr, obj).then((data)=>{
                    ...
                    if(passed == 1){
                        ...
                        this.passTaskAuditLoading = false
                    } else {
                    ...
                    if(data.code == 200){
                        ...
                        this.$store.dispatch('get_people_audit_list',  { limit:this.pageLimit, page:this.$store.getters.getCurrentPage}).then((data)=>{
            ...
            passTaskAudit() {
                ...
                if(flagData.curSelection.length > 0){
                    this.passStorageAuditLoading = true
                    this.$confirm('任务将会通过审核?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                        this.invokePassAudit(flagData, 1, 'pass_audit_task')
                    }).catch(() => {
                        this.passStorageAuditLoading = false
                        this.$message({
                            type: 'warning',
            ...
            confirmNPR(){
                ...
                if(this.notPassType == 'task'){
                    confirmMessage = '任务将不会通过审核?'
                    flagData = this.checkCurrentSelection()
                    dispatchStr = 'pass_audit_task'
                }
                if(flagData.curSelection.length > 0){
